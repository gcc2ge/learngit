...
public class StudyJacob {
	...
	public void setCellString(int tableIndex, int cellRowIdx, int cellColIdx,String value)
			throws Exception {
		// 所有表格
		Dispatch tables = Dispatch.get(doc, "Tables").toDispatch();
		// 要取数据的表格
...
//		Dispatch.put(find, "MatchWildcards", "false");
	...
	public boolean findRegx(String toFindText) {
		if (toFindText == null || toFindText.equals(""))
			return false;
		// 从selection所在位置开始查询
		Dispatch find = word.call(selection, "Find").toDispatch();
...
//		Dispatch.put(find, "MatchWildcards", "True");
	...
	public Variant getText(){
		return Dispatch.get(selection, "Text"); 
	}
